...
function getFinals(item) {
    const Finals = item.filter(function(item){
...
function getYears(callback) {
    return callback.map((team) => team.Year)
}    
...
function getWinnersByYear(winnerCB, yearsCB) {
    const container = []
    for (let i=0; i<winnerCB.length; i++){
        container.push(`In ${yearsCB[i]}, ${winnerCB[i]} won the world cup!`)
    }
    return container
};

console.log("WINNER", getWinnersByYear(getWinners(getFinals(fifaData)), getYears(getFinals(fifaData))))
